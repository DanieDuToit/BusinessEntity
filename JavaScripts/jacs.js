...
var JACS = new function () {// This date is used throughout to determine today's date.
...
//******************************************************************************
    ...
    // basis for themes. Includes are described in comments within the style
    // sheet.
    ...
    document.writeln(
        ...
        '/* Calendar background */' +
        'table.jacs,' +
        'table.jacsStatic            {background-color:  #303030;}' +
        '/* Drag Handle */' +
        ...
        'color:             #666666;}' +
        '/* Today selector */' +
        'td.jacsNow                  {background-color:  #303030;' +
        'color:             #FFFFFF;}' +
        ...
        'color:             #666666;}' +
        'td.jacsNowDisabled          {background-color:  #303030;' +
        'color:             #FFFFFF;}' +
        '</style>');
    ...
    function calAttributes(cal) {
        switch (cal.id) {
            ...
            default:
                ...
                //cal.dateDisplayFormat = 'yyyy-mm-dd';     // e.g. 'MMM-DD-YYYY' for the US
                cal.dateDisplayFormat = 'yyyy-mm-dd';     // e.g. 'MMM-DD-YYYY' for the US
...
// *******************************************************
    ...
    Date.prototype.jacsFormat =
        function (format, monthNames) {
            ...
            for (var i = 0; i <= format.length; i++) {
                if (i < format.length && format.charAt(i) == codeChar) {// If we haven't hit the end of the string and
                    ...
                    if (i < format.length) {// Store the character we have just worked on
    ...
    Function.prototype.jacsRunNext =
        function () {
            var func = this, args = arguments[0];
...
// **************************************
    ...
    if (document.addEventListener) {
        ...
        window.attachEvent('onload', jacsLoader);
    }

    function jacsLoader() {// Define document level event to hide the calendar on click.
        ...
        if (getEl('jacsIElt7')) {
            window.attachEvent('onbeforeunload', defeatLeaks);
        }

        function defeatLeaks() {
            for (var i = 0; i < cals.length; i++) {// Display the week number column (header and row numbers).
                getEl(cals[i] + 'Week_').style.display = '';

                for (var j = 0; j < 6; j++) {
                    getEl(cals[i] + 'Week_' + j).style.display = '';
                }
...
// ****************************************************************************
    ...
    function showMonth(bias, calId) {// Set the selectable Month and Year
        ...
        if (selYears.options.selectedIndex > -1) {
            cal.monthSum = 12 * (selYears.options.selectedIndex) + bias;
        }
        if (selMonths.options.selectedIndex > -1) {
            cal.monthSum += selMonths.options.selectedIndex;
        }
        ...
        if (tmp > -1 && tmp < (12 * cal.dropDownYears)) {
            ...
            if (cal.dates.length == 0) {
                if (cal.active && cal.activeToday) {
                    ...
                    if (document.removeEventListener) {
                        ...
                        now.detachEvent('onclick', stopPropagation);
                    }
                }
                else {
                    now.onclick = null;
                    ...
                    if (document.addEventListener) {
                        ...
                        now.attachEvent('onclick', stopPropagation);
                    }
                }
            }
            else {
            ...
            function setOutput(outputDate, calId) {
                ...
                if (typeof cal.targetEle.value == 'undefined') {
                    ...
                    cal.ele.value = outputDate.jacsFormat(cal.dateFormat, cal.monthNames);
                }
                ...
                if (cal.dynamic) {
                    ...
                    if (typeof cal.onNext != 'undefined' && cal.onNext != null) {
                        cal.onNext();
                    }
                    JACS.show(cal.ele, cal.id, cal.days);
                }

                if (cal.onBlurMoveNext) {// if the target element has a tabIndex look for tabIndex+1
                    ...
                    if (!found) {// find element tabIndices
                        function orderElements() {
                            ...
                            function elementArrays(ele) {
                                for (var i = 0; i < ele.childNodes.length; i++) {
                                    ...
                                    if (tempEle.nodeType == 1 && tempEle.style.display != 'none' && !tempEle.disabled && tempEle.type != 'hidden' &&
                                        tempEle.style.visibility != 'hidden') {
                                        if (tagsToFind.indexOf(tempEle.tagName) > -1) {
                                            if (tempEle.tabIndex > 0) {
                                                tabOrder[tempEle.tabIndex] = tempEle
                                            }
                                            else {
                                                unordered[unordered.length] = tempEle
                                            }
                                        }
                            ...
                            while (tabOrder.length > 0 && tabOrder[0] == null) {
                                tabOrder.shift();
                            }
                        ...
                        for (var i = 0; i < tabSequenced.length; i++) {
                            if (tabSequenced[i] == cal.targetEle) {
                                if (i < (tabSequenced.length - 1)) {
                                    tabSequenced[i + 1].focus()
                                }
                                else {
                                    tabSequenced[0].focus()
                                }
                                break;
                            }
                        }
            ...
            function changeClass(evt) {
                ...
                if (((evt) ? evt.type : event.type) == 'mouseover') {
                    switch (ele.className) {
                        case 'jacsCells':
                            ele.className = 'jacsCellsHover';
                            break;
                        case 'jacsCellsHighlighted':
                            ele.className = 'jacsCellsHighlightedHover';
                            break;
                        case 'jacsCellsExMonth':
                            ele.className = 'jacsCellsExMonthHover';
                            break;
                        case 'jacsCellsWeekend':
                            ele.className = 'jacsCellsWeekendHover';
                            break;
                        case 'jacsCellsHighlightedWeekend':
                            ele.className = 'jacsCellsHighlightedWeekendHover';
                            break;
                        case 'jacsNow':
                            ele.className = 'jacsNowHover';
                            break;
                        case 'jacsInputDate':
                    ...
                    switch (ele.className) {
                        case 'jacsCellsHover':
                            ele.className = 'jacsCells';
                            break;
                        case 'jacsCellsHighlightedHover':
                            ele.className = 'jacsCellsHighlighted';
                            break;
                        case 'jacsCellsExMonthHover':
                            ele.className = 'jacsCellsExMonth';
                            break;
                        case 'jacsCellsWeekendHover':
                            ele.className = 'jacsCellsWeekend';
                            break;
                        case 'jacsCellsHighlightedWeekendHover':
                            ele.className = 'jacsCellsHighlightedWeekend';
                            break;
                        case 'jacsNowHover':
                            ele.className = 'jacsNow';
                            break;
            ...
            function weekNumber(inDate) {// The base day in the week of the input date
                ...
                if (startWeekOne > firstBaseDay) {
                    startWeekOne.setDate(startWeekOne.getDate() - 7);
                }
            ...
            for (var i = 0; i < cells.length; i++) {
                ...
                if (rows.nodeType == 1 && rows.tagName == 'TR') {
                    ...
                    if (cal.weekNumberDisplay) {//Calculate the week number using showDate
                    ...
                    for (var j = 1; j < rows.childNodes.length; j++) {
                        ...
                        if (cols.nodeType == 1 && cols.tagName == 'TD') {
                            ...
                            if ((cal.outOfRangeDisable && (showDate < (new Date(cal.baseYear, 0, 1, 12)) ||
                                ...
                                if ((cal.days.join().search(((j - 1 + (7 * (i * cells.length / 6)) + cal.weekStart) % 7)) > -1) || !cal.dayCells[j - 1 + (7 * ((i * cells.length) / 6))]
                                ) {
                                    disabled = !cal.valuesEnabled;
                                } // Set (Disable or Enable) if the day is passed as a parameter of JACS.show
                                else {
                                    for (var k = 0; k < cal.dates.length; k++) {
...
// *********************************
    ...
    return {
        show: function (ele) {// Check the type of any additional parameters.
            ...
            if (typeof arguments[1] == 'object') {
                ...
                if (source.tagName)    // Second parameter isn't an event it's an element
                {
                    ...
                    if (getEl('jacsIE')) {
                        ...
                        sourceEle.parentNode.addEventListener('click', stopPropagation, false);
                    }
                }
            ...
            if (document.addEventListener) {
                ...
                ele.attachEvent('onkeydown', hideOnTab);
            ...
            if (arguments.length == min) {
                ...
                selectedDays = (typeof arguments[min] == 'object') ? arguments[min] : arguments;
                for (var i = (min | 0); i < selectedDays.length; i++) {
                    if (cal.days.join().indexOf(selectedDays[i]) == -1) {
                        cal.days.push(selectedDays[i]);
                    }
                }
            }
            ...
            if (ele.value) {
                dateValue = ele.value.replace(/^\s+/, '').replace(/\s+$/, '');
            }
            else {
            ...
            if (yearOptions.length == 0 || yearOptions[0].value != cal.baseYear) {
                ...
                for (var i = 0; i < cal.dropDownYears; i++) {
                    yearOptions[i] = new Option((cal.baseYear + i), (cal.baseYear + i));
                }
            }

            if (dateValue.length == 0) {// If no value is entered and today is within the range,
                    ...
                    switch (input.length) {
                        ...
                        {// Year only entry or undelimited date format
                            ...
                            if (cal.dateFormat.indexOf('M') > -1 &&
                                ...
                                seed[1] = cal.defaultToCurrentMonth ? (dateNow.getMonth() + 1).toString() : '6';
                            }
                            ...
                            if (input[0].length == cal.dateFormat.length) {
                ...
                if (seedDate[0] < 100) {
                    seedDate[0] += (seedDate[0] > 50) ? 1900 : 2000;
                }
                ...
                if ((new Date(cal.baseYear, 0, 1)) > cal.seedDate) {
                    ...
                    if ((new Date(cal.baseYear + cal.dropDownYears, 0, 0)) < cal.seedDate) {
                        ...
                        if (cal.strict && cal.fullInputDate &&
                            ...
                            if (cal.showDoesNotExistMsg) {
                                alert(cal.doesNotExistMsg);
                            }
            ...
            for (var i = 0; i < cal.dates.length; i++) {
                if (!((typeof cal.dates[i] == 'object') && (cal.dates[i].constructor == Date))) {
                    if ((typeof cal.dates[i] == 'object') && (cal.dates[i].constructor == Array)) {
                        ...
                        if (pass && (cal.dates[i][0] > cal.dates[i][1])) {
                            cal.dates[i].reverse();
                        }
                    }
                    else {
            ...
            if (monthOptions.length == 0 || (cal.monthNames.join() + ',') != months) {
                ...
                if (cal.monthNames.length < monthOptions.length) {
                    monthOptions.length = cal.monthNames.length;
                }
                ...
                for (var i = 0; i < cal.monthNames.length; i++) {
                    if (i > monthOptions.length - 1) {
                        ...
                        monthOptions[i].innerHTML = cal.monthNames[i];
                    }
                }
            }
            ...
            if (((new Date(cal.baseYear + cal.dropDownYears, 0, 0)) > dateNow &&
                ...
                if ((new Date(cal.baseYear + cal.dropDownYears, 0, 0)) > dateNow &&
                    ...
                    if (cal.clearButton && (ele.readOnly || ele.disabled)) {
                        ...
                        getEl(calId + 'Clear').style.display = 'none';
                        getEl(calId + 'Now').style.textAlign = 'center';
                    }
                }
                else {
                    getEl(calId + 'Clear').style.textAlign = 'center';
                    getEl(calId + 'Clear').style.display = '';
                    getEl(calId + 'Now').style.display = 'none';
                }
            }
            else {
                getEl(calId + 'Foot').style.display = 'none';
            }
            ...
            if (dynamic) {// Check whether or not dragging is allowed and display drag handle if necessary
                ...
                if (cal.xBase.length > 0) {
                    if (isNaN(cal.xBase)) {
                        ...
                        offsetLeft += parseInt(cal.xBase, 10);
                    }
                }
                ...
                if (cal.yBase.length > 0) {
                    if (isNaN(cal.yBase)) {
                        ...
                        offsetTop += parseInt(cal.yBase, 10);
                    }
                }
                else {
                    offsetTop += parseInt(ele.offsetHeight, 10);
                }
                ...
                if (cal.xPosition.length > 0) {
                    if (isNaN(cal.xPosition)) {
                        ...
                        offsetLeft += parseInt(cal.xPosition, 10);
                    }
                }
                ...
                if (cal.yPosition.length > 0) {
                    if (isNaN(cal.yPosition)) {
                        ...
                        offsetTop += parseInt(cal.yPosition, 10);
                    }
                }
        ...
        make: function (calId) {
            ...
            if (dynamic) {
                TABLEjacs.style.zIndex = TABLEjacs.zIndex + 1;
            }
            ...
            INPUTjacsHead1.onclick = function () {
                showMonth(-1, calId);
            }
            ...
            SELECTjacsHead2.onchange = function () {
                showMonth(0, calId);
            }
            ...
            SELECTjacsHead3.onchange = function () {
                showMonth(0, calId);
            }
            ...
            INPUTjacsHead4.onclick = function () {
                showMonth(1, calId);
            }
            ...
            INPUTjacsClearButton.onclick = function () {
                cal.targetEle.value = '';
                hide(calId);
            };
            ...
            if (TABLEjacs.clickToHide) {
                if (document.addEventListener) {
                    ...
                    TABLEjacs.attachEvent('onclick', cancel);
            ...
            if (dynamic) {
                ...
                if (!getEl('jacsSpan' + calId)) {
                    document.writeln("<span id='jacsSpan" + calId + "'></span>");
                }
        ...
        next: function () {
            ...
            if (getEl(calID)) {// Take the arguments to be passed through to the defined function.

                var args = new Array();

                for (var i = argPosition; i < arguments.length; i++) {
